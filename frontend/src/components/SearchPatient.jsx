...
function SearchPatient() {
  ...
  const handleSearch = async () => {
    try {
      const response = await axios.get(`http://localhost:8000/api/v1/doctors/searchPatientsByName?name=${searchName}&doctorUsername=${doctorUsername}`);
    ...
    <div>
      ...
      <div>
        {resultedPatients.map((patient) => (
          <button key={patient._id} id={patient._id} onClick={handleSelectPatient}>{patient.name}</button>
        ))}
      </div>
      {
      selectedPatient && (

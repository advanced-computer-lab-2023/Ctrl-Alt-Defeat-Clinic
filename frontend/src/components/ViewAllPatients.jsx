...
function ViewAllPatients() {
  ...
  const getPatientsWithUpcomingAppointments = async () => {
    try {
      const response = await axios.get(
        `http://localhost:8000/api/v1/appointments/filterPatients?doctorUsername=${doctorUsername}`
      );
  ...
  const handleViewPatients = async () => {
    try {
      const response = await axios.get(
        `http://localhost:8000/api/v1/doctors/viewPatients?doctorUsername=${doctorUsername}`
      );
      setDisplayedPatients(response.data);
    } catch (error) {
  ...
  return (
    <div>
      ...
      <br />
      <br />
      <button id="all" onClick={handleViewPatients}>
        All patients
      </button>
      <button id="upcoming" onClick={getPatientsWithUpcomingAppointments}>
        ...
        <div>
          ...
          <ul>
            {displayedPatients.map((patient) => (
              <li key={patient._id}>
                <button id={patient._id} onClick={handleSelectPatient}>
                  {patient.name}
                </button>
              </li>
            ))}

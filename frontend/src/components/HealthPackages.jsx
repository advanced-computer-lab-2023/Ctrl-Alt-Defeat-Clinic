...
const HealthPackage = () => {
  ...
  useEffect(() => {
    ...
    const fetchPatientHealthPackage = async () => {
      axios.get("http://localhost:8000/api/v1/patients/viewStatusOfHealthPackage", { withCredentials: true })
      .then((response) => {
  ...
  const handleSubscription = async () => {
    ...
    try {
      const response = await axios.post("http://localhost:8000/api/v1/patients/createCheckoutSessionForHp",
      {healthPackageId},
      { withCredentials: true });
  ...
  const subscribeByWallet = async () => {
    const healthPackageId = selectedPackage;
    try {
      const response = await axios.post("http://localhost:8000/api/v1/patients/subscribeToHealthPackageByWallet",
      {healthPackageId},
      { withCredentials: true });
        ...
        {healthPackages.map((healthpackage) => (
              ...
              <br />
          {
            patientHealthPackage.id !== healthpackage._id ?
              <button id={healthpackage._id} onClick={handleClick}>subscribe</button>
              :
              <>
                ...
                <p>
                  { 
                    ...
                    (<>
                    Subscription renewal date is {patientHealthPackage.renewalDate.split('T')[0]}
                    <br />
                    <br />
                    ...
                    : (
                    <>  
                      Subscription end date was {patientHealthPackage.endDate.split('T')[0]}
                      <br />
                      <br />
                      <button id={healthpackage._id} onClick={handleClick}>subscribe again</button>
                    </>
                    )
